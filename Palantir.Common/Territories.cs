namespace Palantir.Common;

public enum ETerritoryType : ushort
{
    Palace_1_10 = 561,
    Palace_11_20,
    Palace_21_30,
    Palace_31_40,
    Palace_41_50,
    Palace_51_60 = 593,
    Palace_61_70,
    Palace_71_80,
    Palace_81_90,
    Palace_91_100,
    Palace_101_110,
    Palace_111_120,
    Palace_121_130,
    Palace_131_140,
    Palace_141_150,
    Palace_151_160,
    Palace_161_170,
    Palace_171_180,
    Palace_181_190,
    Palace_191_200,

    HeavenOnHigh_1_10 = 770,
    HeavenOnHigh_11_20 = 771,
    HeavenOnHigh_21_30 = 772,
    HeavenOnHigh_31_40 = 782,
    HeavenOnHigh_41_50 = 773,
    HeavenOnHigh_51_60 = 783,
    HeavenOnHigh_61_70 = 774,
    HeavenOnHigh_71_80 = 784,
    HeavenOnHigh_81_90 = 775,
    HeavenOnHigh_91_100 = 785,

    EurekaOrthos_1_10 = 1099,
    EurekaOrthos_11_20 = 1100,
    EurekaOrthos_21_30 = 1101,
    EurekaOrthos_31_40 = 1102,
    EurekaOrthos_41_50 = 1103,
    EurekaOrthos_51_60 = 1104,
    EurekaOrthos_61_70 = 1105,
    EurekaOrthos_71_80 = 1106,
    EurekaOrthos_81_90 = 1107,
    EurekaOrthos_91_100 = 1108,

    PilgrimsTraverse_1_10 = 1281,
    PilgrimsTraverse_11_20 = 1282,
    PilgrimsTraverse_21_30 = 1283,
    PilgrimsTraverse_31_40 = 1284,
    PilgrimsTraverse_41_50 = 1285,
    PilgrimsTraverse_51_60 = 1286,
    PilgrimsTraverse_61_70 = 1287,
    PilgrimsTraverse_71_80 = 1288,
    PilgrimsTraverse_81_90 = 1289,
    PilgrimsTraverse_91_100 = 1290,
}

public static class Territories
{
    public static bool IsDeepDungeon(ushort territory) => Enum.IsDefined((ETerritoryType)territory);

    public static bool IsDeepDungeon(uint territory) =>
        territory <= ushort.MaxValue && IsDeepDungeon((ushort)territory);
}
